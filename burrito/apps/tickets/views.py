...
async def my_reports(Authorize: AuthJWT = Depends(get_auth_core())):
...
async def to_me(Authorize: AuthJWT = Depends(get_auth_core())):
    """View all reports created by other users sended to current user"""
    Authorize.jwt_required()


async def followed(Authorize: AuthJWT = Depends(get_auth_core())):
    """View all reports that current user have followed"""
    Authorize.jwt_required()


async def create_new_report(Authorize: AuthJWT = Depends(get_auth_core())):
    """Create report"""
    Authorize.jwt_required()
